...
if(isset($_GET['id'])){
    ...
    if($qry->num_rows >0){
        ...
        if($from_order == 1){
            ...
            // Check if bo_id exists in GET parameters before using it
            if(isset($_GET['bo_id'])) {
...
<div class="card card-outline card-primary">
    <div class="card-header">
        <h4 class="card-title"><?php echo !isset($id) ? "Receive Order from ".(isset($po_code) ? $po_code : '') : 'Update Received Order' ?></h4>
    </div>
    <div class="card-body">
        <form action="" id="receive-form">
            <input type="hidden" name="id" value="<?php echo isset($id) ? $id : '' ?>">
            <input type="hidden" name="from_order" value="<?php echo isset($bo_id) ? 2 : 1 ?>">
            <input type="hidden" name="form_id" value="<?php echo isset($bo_id) ? $bo_id : (isset($po_id) ? $po_id : '') ?>">
