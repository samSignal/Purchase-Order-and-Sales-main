			...
			<table class="table table-bordered table-stripped">
                    ...
                    <tbody>
                        ...
                        while($row = $qry->fetch_assoc()):
                            ...
                            $code = '';
                            if($row['from_order'] == 1){
                                $po_result = $conn->query("SELECT po_code from `purchase_order_list` where id='{$row['form_id']}' ");
                                $po_row = $po_result->fetch_assoc();
                                $code = $po_row ? $po_row['po_code'] : 'N/A';
                            }else{
                                $bo_result = $conn->query("SELECT bo_code from `back_order_list` where id='{$row['form_id']}' ");
                                $bo_row = $bo_result->fetch_assoc();
                                $code = $bo_row ? $bo_row['bo_code'] : 'N/A';
                            }
                        ?>
                            <tr>
